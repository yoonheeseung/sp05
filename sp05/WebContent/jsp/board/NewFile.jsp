 ...
 <input type="hidden" name="no" value="${bc.no}" />
  <table align="center" border="1">
   ...
   <tr>
    <th>주소</th>
    <td>
     <input name="addr" id="addr" size="36" 
     value="${bc.addr}" />
    </td>
   </tr>
   <tr>
    <th>비밀번호</th>
    <td>
     <input type="password" name="pass" id="pass" size="14" />
    </td>
   </tr>
   <tr>
    <th>성별</th>
    ...
    <%--radio는 단 하나만 선택가능하다.checked속성을 
    사용하면 미리 체크됨. --%>
    </td>
   </tr>
   <tr>
    ...
    <c:forEach var="city" items="${c}" >
     <option value="${city}"
     <c:if test="${bc.city == city}">
      ${'selected'} </c:if>>${city}</option>
    </c:forEach>
    </select>
    </td>
   </tr>
   ...
   <th>자기소개</th>
   <td>
    <textarea name="cont" id="cont"rows="8" 
    cols="34">${bc.cont}</textarea>
   </td>
   </tr>
   <tr>
   <th colspan="2">
